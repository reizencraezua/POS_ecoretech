...
@section('page-description', 'Edit job order details')
...
@section('content')
<div class="space-y-6">
    <form method="POST" action="{{ route('cashier.orders.update', $order) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button type="button" id="add-item" class="bg-maroon hover:bg-maroon-dark text-white px-4 py-2 rounded-lg font-medium transition-colors inline-flex items-center">
            ...
            <div id="items-container" class="space-y-4">
                @foreach(old('items', $order->details->map(function($detail) {
                    return [
                        ...
                        'size' => $detail->size,
                        'price' => $detail->price,
                        'layout' => $detail->layout ? 'on' : '',
                        'layoutPrice' => $detail->layout_price
                    ];
                ...
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                        <div>
                            ...
                            <select name="items[{{ $index }}][type]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon" onchange="loadItems(this, {{ $index }})">
                        ...
                        <div>
                            ...
                            <select name="items[{{ $index }}][id]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon" required>
                                <option value="">Select Item</option>
                                @if($item['type'] == 'product')
                                    @foreach($products as $product)
                                        <option value="{{ $product->product_id }}" data-price="{{ $product->product_price }}" {{ $item['id'] == $product->product_id ? 'selected' : '' }}>
                                            {{ $product->product_name }}
                                        </option>
                                    @endforeach
                                @elseif($item['type'] == 'service')
                                    @foreach($services as $service)
                                        <option value="{{ $service->service_id }}" data-price="{{ $service->service_price }}" {{ $item['id'] == $service->service_id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="items[{{ $index }}][quantity]" min="1" value="{{ $item['quantity'] }}" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Unit</label>
                            <select name="items[{{ $index }}][unit]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon">
                                <option value="">Select Unit</option>
                                @foreach($units as $unit)
                                    <option value="{{ $unit->unit_code }}" {{ $item['unit'] == $unit->unit_code ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="items[{{ $index }}][price]" step="0.01" min="0" value="{{ $item['price'] }}" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Layout</label>
                            <div class="flex items-center space-x-2">
                                <input type="checkbox" name="items[{{ $index }}][layout]" value="1" class="rounded border-gray-300 text-maroon focus:ring-maroon" {{ $item['layout'] ? 'checked' : '' }}>
                                <input type="number" name="items[{{ $index }}][layoutPrice]" step="0.01" min="0" value="{{ $item['layoutPrice'] }}" placeholder="Layout Price"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-maroon focus:border-maroon">
                            </div>
                        </div>
                        <div>
...
document.addEventListener('DOMContentLoaded', function() {
    let itemCount = {{ count(old('items', $order->details->toArray())) }};
