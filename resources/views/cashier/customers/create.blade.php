...
@section('page-description', 'Create a new customer record')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        ...
        <form method="POST" action="{{ route('cashier.customers.store') }}" class="p-6">
            @csrf
            ...
            <div class="mt-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon focus:border-maroon @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
